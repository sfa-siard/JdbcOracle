...
public class OracleDatabaseMetaData
    ...
    private String getDataTypeCase(String sDataType, String sTypeCode)
        ...
        for (Iterator<String> iterTypeName = mapDataType.keySet()
                                                        .iterator(); iterTypeName.hasNext(); ) {
    ...
    private StringBuilder createGetTablesQuery(String schemaPattern, String tableNamePattern, String[] types) throws SQLException {
        ...
        StringBuilder selector = new StringBuilder("SELECT\r\n")
                ...
                .append("  O.OWNER AS TABLE_SCHEM,\r\n")
                .append("  O.OBJECT_NAME AS TABLE_NAME,\r\n")
                .append(getTableTypesCaseExpression())
                .append(" AS TABLE_TYPE,\r\n")
                .append("  TC.COMMENTS AS REMARKS,\r\n")
                ...
                .append(_sQUERY_TEXT)
                .append("\r\n");
        return getQuery(selector, condition);
    }

    private StringBuilder getCondition(String schemaPattern, String tableNamePattern, String[] types) throws SQLException {
        StringBuilder condition = new StringBuilder("O.OBJECT_NAME LIKE ");
        condition.append(SqlLiterals.formatStringLiteral(tableNamePattern))
                 .append(" ESCAPE ")
                 .append(SqlLiterals.formatStringLiteral(getSearchStringEscape()))
                 .append("\r\n");
        if (schemaPattern != null) {
            condition.append("AND O.OWNER LIKE ")
        ...
        if (types != null) {
            ...
            if (!objectTypes.isEmpty()) {
                ...
                for (String objectType : objectTypes) {
                    ...
                    condition.append("(O.OBJECT_TYPE = ")
                             .append(SqlLiterals.formatStringLiteral(objectType));
                    if (objectType.equals(TABLE)) {
                        condition.append(" AND NOT T.TABLE_NAME IS NULL");
                    } else if (objectType.equals(VIEW)) {
                        condition.append(" AND NOT V.VIEW_NAME IS NULL");
                    }
                    condition.append(")\r\n");
                }
                condition.append(")\r\n");
            ...
            condition.append(" AND \r\n")
                     .append(getOracleMaintainedCondition(isSystem))
                     .append("\r\n");
            if (!isSystem) {
                condition.append("AND O.GENERATED = ")
                         .append(SqlLiterals.formatStringLiteral("N"))
                         .append("\r\n");
            }

            String temporary = isTemporary ? "Y" : "N";
            condition.append("AND O.TEMPORARY = ");
            condition.append(SqlLiterals.formatStringLiteral(temporary));
            condition.append("\r\n");
        }
        return condition;
    }

    private String getTableTypesCaseExpression() throws SQLException {
        String tableTypesCase = "CASE\r\n" + "  WHEN O.OBJECT_TYPE = " +
                SqlLiterals.formatStringLiteral(TABLE) +
                " AND O.TEMPORARY = " +
                SqlLiterals.formatStringLiteral("N") +
                " AND " +
                getOracleMaintainedCondition(false) +
                " THEN " +
                SqlLiterals.formatStringLiteral(TABLE) +
                "\r\n" +
                "  WHEN O.OBJECT_TYPE = " +
                SqlLiterals.formatStringLiteral(TABLE) +
                " AND O.TEMPORARY = " +
                SqlLiterals.formatStringLiteral("Y") +
                " AND " +
                getOracleMaintainedCondition(false) +
                " THEN " +
                SqlLiterals.formatStringLiteral(GLOBAL_TEMPORARY) +
                "\r\n" +
                "  WHEN O.OBJECT_TYPE = " +
                SqlLiterals.formatStringLiteral(TABLE) +
                " AND O.TEMPORARY = " +
                SqlLiterals.formatStringLiteral("N") +
                " AND " +
                getOracleMaintainedCondition(true) +
                " THEN " +
                SqlLiterals.formatStringLiteral(SYSTEM_TABLE) +
                "\r\n" +
                "  WHEN O.OBJECT_TYPE = " +
                SqlLiterals.formatStringLiteral(VIEW) +
                " AND O.TEMPORARY = " +
                SqlLiterals.formatStringLiteral("N") +
                " AND " +
                getOracleMaintainedCondition(false) +
                ...
                "\r\n" +
                "  WHEN O.OBJECT_TYPE = " +
                SqlLiterals.formatStringLiteral(SYNONYM) +
                " AND O.TEMPORARY = " +
                SqlLiterals.formatStringLiteral("N") +
                " AND " +
                getOracleMaintainedCondition(false) +
                ...
                "\r\n" +
                "ELSE NULL END";
        return tableTypesCase;
    }

    private static StringBuilder getQuery(StringBuilder selector, StringBuilder condition) {

        return selector
                .append("FROM ALL_OBJECTS O\r\n")
                .append("  INNER JOIN " + sTABLE_ALL_USERS + " " + sALIAS_ALL_USERS + "\r\n")
                .append("  ON (O.OWNER = " + sALIAS_ALL_USERS + "." + sCOLUMN_USERNAME + ")\r\n")
                .append("  LEFT JOIN ALL_TABLES T\r\n")
                .append("  ON (O.OWNER = T.OWNER AND \r\n")
                .append("      O.OBJECT_NAME = T.TABLE_NAME AND\r\n")
                .append("      O.OBJECT_TYPE = 'TABLE' AND\r\n")
                .append("      T.NESTED = 'NO')\r\n")
                .append("  LEFT JOIN ALL_VIEWS V\r\n")
                .append("  ON (O.OWNER = V.OWNER AND\r\n")
                .append("      O.OBJECT_NAME = V.VIEW_NAME AND\r\n")
                .append("      O.OBJECT_TYPE = 'VIEW')\r\n")
                .append("  LEFT JOIN ALL_TAB_COMMENTS TC\r\n")
                .append("  ON (O.OWNER = TC.OWNER AND\r\n")
                .append("      O.OBJECT_NAME = TC.TABLE_NAME AND\r\n")
                .append("      O.OBJECT_TYPE = TC.TABLE_TYPE)\r\n")
                .append("WHERE\r\n")
                .append(condition.toString())
                .append("ORDER BY TABLE_TYPE, TABLE_CAT, TABLE_SCHEM, TABLE_NAME");
    }
    ...
    public ResultSet getTableTypes()
        ...
        for (int iTableType = 0; iTableType < TABLE_TYPES.length; iTableType++) {
            if (iTableType > 0)
                sbSql.append("UNION\r\n");
            sbSql.append("(SELECT ");
            sbSql.append(SqlLiterals.formatStringLiteral(TABLE_TYPES[iTableType]));
